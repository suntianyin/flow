...
<script>
    ...
    function AcceptClick() {
        ...
        window.setTimeout(function () {
            ...
            $('.sys_spec_text .selected a').each(function () { roleid += $(this).attr('id') + ","; });
            roleid = roleid.substr(0,roleid.length-1);
            var postData = {"roleid":roleid,"groupid":groupid};
            $.ajax({                
                url: "grouproleupdate",
                type: "POST",
                data: postData,                
...
<div class="border" style="margin: 1px;">
    ...
    <div class="ScrollBar" id="MemberList" style="height: 225px;">
        <ul class="sys_spec_text">
            ...
            <li title="${listthis.name }"><a id="${listthis.id }" class="selected"><img src="${ctx}/images/role.png">${listthis.name }</a><i></i></li>
                </#list >
        </ul>
    </div>
</div>
