 ...
 */
/* 表单验证使用实例！  */
//验证role_code字段必须以ROLE_开头并且不为空
function isRoleCodeAndNotNull(obj){
	controlObj = $.trim(obj);
    ...
    $(obj).find("[datacol=yes]").each(function () {
        if ($(this).attr("checkexpession") != undefined) {
            ...
            switch ($(this).attr("checkexpession")) {
                ...
                case "isRoleCodeAndNotNull":
                	{
	                    if (!isRoleCodeAndNotNull(value, $(this).attr("eqvalue"))) {
	                        Validatemsg = $(this).attr("err") + "不能为空且必须为ROLE_数字格式！\n";
	                        Validateflag = false;
	                        tipCss($(this), Validatemsg); return false;
	                    }
	                    break;
