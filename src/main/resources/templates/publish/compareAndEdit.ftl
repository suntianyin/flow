        ...
        <#list compareEntityList as compareEntity>
            <#if compareEntity.info == '0'>
                <tr class="gradeA odd" role="row" style="background-color: red">
                    ...
                    <#elseif compareEntity.fieldName == '是否有cebx' || compareEntity.fieldName = '是否有流式内容' || compareEntity.fieldName = '是否发布' ||  compareEntity.fieldName = '流式内容是否优化' || compareEntity.fieldName = '更新时间' || compareEntity.fieldName = '创建时间' || compareEntity.fieldName = '是否公版' || compareEntity.fieldName = '阿帕比图书metaid'>
                        <td>${compareEntity.metaValue}</td>
