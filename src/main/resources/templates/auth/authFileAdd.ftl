    ...
    <script>
        ...
        function AcceptClick() {
            ...
            window.setTimeout(function () {
                ...
                formData.append('caid',$('#caid').val().trim());
                $.ajax({
                    url: RootPath() + "/auth/authFileInsert",
                    type: "POST",
                    data: formData,
                    cache: false,
                    processData: false,
                    ...
                    success: function (data) {
                        if (data == "success") {
                            ...
                            tipCss('版权协议', '保存失败，联系管理员！');
                        } else if (data == "exist") {
                            var note = "注：您确定要覆盖当前授权协议附件？";
                            confirmDialog("温馨提示", note, function (r) {
                                if (r) {
                                    ...
                                    window.setTimeout(function () {
                                        try {
                                            $.ajax({
                                                url:  RootPath() + "/auth/authFileInsert2",
                                                type: "POST",
...
<form id="form1" enctype="multipart/form-data" method="post">
    <div class="ScrollBar" style="margin: 1px; overflow: hidden;">
        <div id="basic" class="tabPanel">
            <table class="form">
                <tr>
                    <input id="caid" name="caid" type="hidden"  value="${caid!""}"/>
