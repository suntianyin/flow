...
//    @Autowired
    ...
    public synchronized ResultEntity addBatch(@RequestBody Batch batch){
        try{
            if (StringUtils.isBlank(batch.getManager())
                    ...
                    || StringUtils.isBlank(batch.getOutUnit())
     ...
     * @return
     */
    @PostMapping("/editBatchState")
    @ResponseBody
    public ResultEntity editBatchState(@RequestBody Batch batch){
        try{
            if (StringUtils.isBlank(batch.getId())
                    || batch.getBatchState() == null
                    ||
                    (batch.getBatchState() != BatchStateEnum.DUPLICATE_CHECKED
                            && batch.getBatchState() != BatchStateEnum.PROCESSING
                            && batch.getBatchState() != BatchStateEnum.COMPLETED)
                    ){
    ...
    public ResultEntity uploadBooklistAudit(@RequestParam("batchId") String batchId){
        try{
            ...
            if (list != null || !list.isEmpty()){
                ...
                paramBatch.setBatchState(BatchStateEnum.INPUT_COMPLETED);
                if (batchService.updateBatchStateAndBibliothecaState(paramBatch,
                        null, BibliothecaStateEnum.WAITING_AUDIT)) {
    ...
    public ResultEntity uploadMakeAudit(@RequestParam("batchId") String batchId){
        try{
            ...
            if (list != null || !list.isEmpty()){
                ...
                paramBatch.setUpdateTime(new Date());
                //当 提交书单审核时，则设置 当前批次为 书单录入完成
                paramBatch.setBatchState(BatchStateEnum.PROCESSED_WAITING_AUDIT);
                if (batchService.updateBatch(paramBatch) == 1){
